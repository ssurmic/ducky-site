...
async function fixture({status=200,error,uid=51,pause=false}={}){
 ...
 globalThis.fetch=async(raw,opts={})=>{
  ...
  if(path==='/auth/link/telegram'){
   ...
   if(pause)return new Promise(resolve=>{release=()=>resolve(result);started();});return result;
  }
  throw Error('Unexpected '+path);
 };
 return {url,intent,calls,requestStarted,release:()=>release(),start(){history.replaceState(null,'',url);store.set('token',null);store.set('me',null);return auth.boot();}};
}
...
test('an account switch during a linking request ignores its late result and cannot hydrate the new account',{timeout:10000},async()=>{
 const f=await fixture({pause:true});const pending=f.start();await f.requestStarted;
