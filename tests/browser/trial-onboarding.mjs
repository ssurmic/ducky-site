...
const output='reports/trial-onboarding';await mkdir(output,{recursive:true});
const results=[];
for(const [engine,type] of Object.entries({chromium,webkit})){
 ...
 for(const width of [320,390,430,1440]){
  ...
  const context=await browser.newContext({ignoreHTTPSErrors:true,viewport:{width,height:width<700?844:1000},colorScheme:width===430?'dark':'light',reducedMotion:width===320?'reduce':'no-preference',storageState:{cookies:[],origins:[{origin:'https://127.0.0.1:8766',localStorage:[{name:'ducky.token',value:account.token}]}]}});
  ...
  const result={label,engine,width,real_device:false,data:'synthetic shared research; real local auth/watchlist/trial/onboarding APIs'};
  try{
   await page.goto('https://127.0.0.1:8766/'+lang+'/app/#/watchlist');
   ...
   await page.locator('.tour-route').click();
   await page.locator('[data-tour="calendar.day"]').first().click();
