...
test('signed-out entry point mounts a password reset link and can request a new link', async () => {
  ...
  const dom = new JSDOM('<a data-lang-toggle href="/zh/app/#/reset?token='+secret+'">中文</a><main class="app-main"><div id="view"></div></main>', {
    url: 'https://ducky.test/en/app/#/reset?token=' + secret
  });
  ...
  assert.equal(document.querySelector('[data-lang-toggle]').getAttribute('href'), '/zh/app/#/reset');
