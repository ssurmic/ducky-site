...
Production integration passed with frontend `36a8f8a` / Pages `07abd0d9` and backend `a6a39bf`. Current AVGO graph `se-graph:d34815d8f030955fdf5b077fc0e9ec7a` carries `refresh_pending`, pointing to original analysis snapshot `se-graph:d3292f03eeb46272d5556c63a4b49e55`. On the real production page, the paragraph remains readable with the explicit previous-analysis label and actual 10:48 UTC generation time. Expanded analysis citation 4 opens the 3-mention record observed 10:01 UTC, with an analysis-time notice. The latest map node separately opens 4 mentions observed 11:01 UTC without that notice. The source is the same logical node, but each control resolves to its correct historical value. Actual 393px viewport/document widths match. This verification used no fixture and no new model run. Final integrated checks: 399 tests, copy lint, 1,344 links.

The COIN video header NET/AMZN direction concern was also closed without a code change: expanding the remaining three source sections reveals separate verified bullish claims at 25:10 (NET, `claim:be0fc14c3687f4f1a7f9cc15`) and 23:55 (AMZN, `claim:f3fffd1d54234e5ddb7c0824`). The neutral mention rows are different spans at 26:03 in the same video. Those mentions must not erase the independently grounded viewpoints.

NVDA source correction was independently confirmed in the production store at 11:28:19.625736 UTC: `claim:7ce209bd72de40349998fe26` now preserves the original top-stock-to-buy viewpoint as well as watching closely. The independent five-check source-note review passed with no issues. The original author, September 7 publication, source video `qleHy1oNdjU`, 585.4–595.08 second passage and point ID remain unchanged. A new analysis job started only after that source correction was published.

The corrected NVDA source also passed the real map → Parkev video → same-point map round trip at 393/393 CSS px. The video header and focused span retain the bullish opinion while the dated source retains the original watching intent. This source receipt does not establish that the subsequent analysis job has completed.

### Direct source reads

Repeated production source navigation exposed another dependency: an exact video route waited for all 483 recent creator posts before requesting its indexed source. Retry recovered it, but the unrelated catalogue was unnecessary for opening one source. Focused source routes now read the existing API-gated `/kol/{creator}/posts/{post}` endpoint directly. Returning to the creator's videos, the directory or another workspace remounts the normal complete data source. The focused page omits misleading catalogue counts and an unavailable aggregate overview. Exact source refreshes remain GET-only and participate in the existing source-change notice; unrelated creator job progress no longer reloads the catalogue on that page. An API 402 shows the Pro access link rather than a retry loop.

Mounted regression coverage verifies exact-source recovery without a feed request, preserved point/source focus, complete following-feed navigation afterwards, API access denial and the refresh endpoint allowlist. All 400 frontend tests pass before integration with the concurrent caption-correction display change.

A second actual AVGO source inspection at 393/393 CSS px confirmed the same video keeps `$AVGO 看多` and the 16:13 valuation opinion, while its 10:06 revenue/customer point remains background and the separate 10:04 mention remains nondirectional. Those distinct spans are not flattened to one sentiment.

Direct-source release `ab05ec31442a992ebea8b50aab819b25cf4f0389`, Pages `3b047e38`, integrates the separately reviewed caption-correction display. **401 frontend tests, copy lint and 1,344 links passed.** Real production AVGO opened one video without the aggregate overview or catalogue count; the 16:13 point retained its corrected terminology, an explicitly separate original-caption disclosure and exact source link. “查看这位博主的全部视频” returned to the normal creator page with 30 recent video cards, its aggregate overview and the 55-video archive control. Both routes measured 393/393 CSS px. This browser check did not use synthetic data.

### Real NVDA analysis accepted

`evidence-analysis/17` job `se-analysis:a7922fd7b5326f6f0c5d1eb80199a951` completed at **2026-09-08 11:51:18.406068 UTC**, with independent review approved, all six checks true and no issues. Published graph `se-graph:f44838998562911312dbec9163347136`; evidence version `ec9b2a3a468e8f624e0bba284a5cc050b52cf6b7e52215587ee0d1553fb0d024`. The writer reused the same-evidence valid sections from the failed v16 draft and repaired the required two-clause overview before the complete independent review. No invalid draft was promoted directly.

Production Chinese and English at 393px showed the real 11:51 UTC paragraph immediately, with reasons and risks initially collapsed. Citation 1 opens Parkev's September 7 bullish opinion at **9:45–9:55**, preserving publication and first observation separately. Citation 3 opens Situational Awareness LP's **June 30 quarter-end** record, with August 14 filing date and the explicit limitations that this is neither a trade ledger nor today's holding. Expanded-analysis citation 54 opens **41 Reddit mentions, direction unknown**, despite being outside the initial six cards. The current source → exact Parkev video → same `claim:7ce209bd72de40349998fe26` map-dialog round trip passed on the new direct-source page. Actual mobile 393/393 and English desktop 2592/2592 viewport/document widths match.

The wording remains attributed and dated. The current English overview is concise but still somewhat headline-like, and the discussion-evidence watch item could be plainer in a future normal background generation; the frontend does not rewrite already reviewed source content. There was no physical Safari/device test. The evidence-analysis, supporting-evidence and ticker-brief timers were independently confirmed active after acceptance; no timer was paused for the final v17 run.
