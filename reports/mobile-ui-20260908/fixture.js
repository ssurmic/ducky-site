...
window.fetch=async(url,opts={})=>{const parsed=new URL(url,location.origin);if(parsed.origin!==location.origin)throw Error('QA blocks remote requests');let p=parsed.pathname;if(p.startsWith('/qa-api')){qaCalls.push({path:p,method:opts.method||'GET'});
 ...
 if(p.includes('/bars/'))return json({bars:Array.from({length:80},(_,i)=>{const c=100+Math.sin(i/5)*12+i/8;return {t:new Date(Date.UTC(2026,5,1+i)).toISOString().slice(0,10),o:c-1,h:c+3,l:c-3,c,v:10000};})});
 if(p.includes('/public/company/'))return json({status:'unavailable'});
 if(p.includes('/snapshot/')){if(kind==='loading')return new Promise(()=>{});if(kind==='error')return json({error:'unavailable'},503);return json({ok:true,spot:100,built_at:'2026-09-06T00:00:00Z',tech:{rsi_d:24,rsi_w:42,rsi_m:61,dd_pct:-18.2,vs_50dma:3.4,vs_200dma:-2.5,oversold:true},retrace:{d20:{pos:.2,lo:91.12,hi:111.24}},rs:{benchmark:'SEMICONDUCTOR',excess20:-4.2,label:'lagging'},vol:{iv:42.1,hv:31.2,ratio:1.35,label:'rich'},gamma:{call_wall:110,put_wall:90,flip:100,regime:'negative'},expected:{low:90,high:110,move_pct:10,expiry:'2026-09-18'},company_context:{status:'ready',company:'LOCAL FIXTURE Semiconductor Corporation',label_zh:'半导体',label_en:'Semiconductors',business_zh:'仅测试公司说明及布局，不代表实际公司数据。',business_en:'Synthetic UI layout fixture, not actual company data.',peers:['AMD','INTC'],sources:[{url:'https://example.com/fixture',title:'LOCAL FIXTURE evidence'}]}});}
