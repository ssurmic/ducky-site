...
网站 `f76a4be8` / Pages `23a167f2` 首次整合上线，保留同轮导航与博主搜索改动，232 Node tests通过。最后同日拥挤排序修复以 `13690daf` / Pages `69ac8b32` 发布；功能233 tests通过，最终两行通知检查时段说明再经20页构建、1761文件copy和748links验证。

Root真实Chrome登录会话确认：BE官方雷达记录与来源可读，日历深链在390px为14天/两周默认；9/21实际42条指数调整中BE预览和展开第一，modal仍有全部42行，scrollWidth=390。新来源收件箱真实为空，旧博主示例仍明确标历史演示；真实9股票热图和收件箱均无横向溢出。官方指数组合条件仅做临时预览，匹配33股票含BE，没有保存筛选、关注、提醒或开启通知。日历view/model及页面实际引用的versioned CSS与构建字节相同，测试完已恢复浏览器视口。此为Chrome响应尺寸检查，非实体iOS Safari。

## 同日 42 条指数事件补测（生产验收发现，2026-09-07）

生产中的 9/21 有大量指数事件，单条 BE fixture 未暴露代码预览和排序问题。已将输入 BE 故意放在第 42 条，前三条为 AGNC / AMSF / BKE，另有 38 条合成边界记录；只关注 AMSF，深链指定未关注的 BE。`crowded=1&tier=free&view=calendar` 可复现。

统一日排序保留休市/提前收市在前，其后是合法深链股票、宏观、其它关注股票、其余事件；同组保留来源顺序。指数预览使用股票代码，两周 aria 最多列三条、每条 80 字符，同时保留完整总数。未自动开启只看自选，未删除同日其它事件。

实际 Chrome：320 en light、390 zh dark，预览均为 BE / AMSF / 另外 40 项，弹窗完整 42 条，首四条 BE / AMSF / AGNC / BKE，末条 F37。320/320、390/390 无横向溢出；只有公开日历与静态 seasonality GET，无私有 context/links 请求、无写入、fixture 未捕获运行错误。见 [英文预览](official-ui-20260907/en-light-crowded-preview320.png)、[英文弹窗](official-ui-20260907/en-light-crowded-modal320.png)、[中文预览](official-ui-20260907/zh-dark-crowded-preview390.png)、[记录数量与请求](official-ui-20260907/crowded-day.json)。

本次全量 Node 233/233 通过；定向 16/16。回归覆盖日期顺序、休市与宏观顺序、合法/非法 ticker、无数据丢失或输入修改、两周/弹窗/月/列表一致排序和免费门禁。测试仍是隔离数据，非历史通知证据。
