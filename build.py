...
def load_home_proof() -> dict:
    ...
    try:
        return home_proof.load(PUBLIC / 'home-proof.json')
    except (OSError, ValueError, UnicodeError) as exc:
        fail('public/home-proof.json: ' + str(exc))
...
def write_config_js(cfg: dict, version: str, filename: str = 'config.js') -> None:
    data = {
...
def main() -> None:
    ...
    for tpl_name, rel in pages:
        ...
        for lang in LANGS:
            ...
            if tpl_name=='app.html' and cfg.get('product_focus_preview') is True:
                ...
                # unchanged until actual user tasks have been evaluated.
                trial={**cfg,'product_focus':True}
                preview_ctx={**ctx,'cfg':trial,
                    'alt_url':page_url(ctx['other_lang'],'app/preview/index.html')}
                preview_html=tpl.render(**preview_ctx).replace('src="/config.js"','src="/focus-config.js"')
                preview_html=version_assets(preview_html,version,app_version)
                validate_app_strings(preview_html,tables[lang],lang)
    ...
    if cfg.get('product_focus_preview') is True:
        write_config_js({**cfg,'product_focus':True},version,'focus-config.js')
    write_glossary(DIST)
