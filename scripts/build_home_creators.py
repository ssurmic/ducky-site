...
Input: the newest public/media/creator-points-<date>.json — a read-only export of every attributed
creator view (ticker, stance, publication time, bilingual title, source URL) plus the creator roster
(name, language, videos tracked), produced on the backend host by scripts/export_creator_points.py.
...
larger moves. It is an example per creator, not a hit rate; the page says so and links to the app,
where every view and its outcome is listed. Creators without a qualifying view keep their roster row.
Run with an interpreter that has yfinance:
  ~/dev/ducky-bot/.venv/bin/python scripts/build_home_creators.py --as-of 2026-09-12
"""
...
def main():
    ...
    src = sorted((ROOT / 'public/media').glob('creator-points-????-??-??.json'))[-1]
    doc = json.loads(src.read_text())
    kols = {k['name']: k for k in doc['kols']}
    points = [p for p in doc['points'] if p.get('author') in kols and p.get('ticker') and p.get('source_url', '').startswith('https://')]
    closes = {}
    ...
    PICKS = {
        ...
        'New Money': ('GOOGL', '2026-08-17', None),
    ...
    out = dict(as_of=args.as_of, source=src.name,
               presentation='One direction-consistent example view per creator with the price change after publication. '
                            'Not a hit rate or a ranking; the app lists every view and its outcome.',
