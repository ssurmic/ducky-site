...
export function renderMarketContext(doc,{preview=false,watches=[]}={}) {
  const box=el('section.card.market-context',el('div.market-context-heading',
    el('div',el('span.event-eyebrow',s('market.eyebrow')),el('h2',s('market.title')))));
  box.append(el('p.muted',s('market.description')));
  if(doc.status==='unavailable'||!doc.topics?.length) {
    box.append(el('p.data-notice',s('market.unavailable')));return box;
  }
  box.append(el('p.small.muted',s('market.observed',{date:doc.observed_at||'—'})));
  if(doc.status==='stale')box.append(el('p.data-notice',{role:'status'},s('market.stale')));
  const coverage=doc.coverage||{};
  box.append(el('p.small.market-coverage',s('market.coverage',{news:coverage.headlines||0,creators:coverage.creator_views||0,contracts:coverage.prediction_contracts||0,sources:coverage.distinct_publishers||0})),
  ...
  if(!preview)box.append(el('p.small.market-watch-context',s(watchSet.size?(doc.topics.some(t=>relevant(t).length)?'market.watch_matches':'market.watch_unmatched'):'market.watch_empty')));
  ...
  for(const topic of [...doc.topics].sort((a,b)=>Number(relevant(b).length>0)-Number(relevant(a).length>0))) {
    const card=el('article.market-topic',el('h3',localized(topic,'label')),
      el('p.small.muted',s('market.topic_sources',{n:topic.source_count})+' · '+topic.latest_at));
    ...
    if(synthesis&&!preview) {
      card.append(el('span.event-eyebrow',s('market.ducky_inference')),el('p',localized(synthesis,'summary')),
        el('h4',s('market.counter')),el('p.small',localized(synthesis,'unknown')),
    ...
    if(refs.length&&!preview) {
      ...
      for(const item of refs) {
        const url=source(item.source_url);const row=el('article.market-evidence-item',
          ...
          el('p.small.muted',item.publisher+' · '+item.published_at),
    ...
    if(tickers.childElementCount)card.append(tickers,el('p.small.muted',s('market.ticker_basis')));
    grid.append(card);
  }
  box.append(grid);
