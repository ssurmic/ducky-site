...
export function macroTiles(doc){
  ...
  return [
    {key:'liquidity',label:s('today.macro_liquidity'),value:OK(latest.funding_score)?num(latest.funding_score,0):'—',unit:s('today.macro_score_unit'),
     tone:regime==='supportive'?'up':regime==='adverse'?'down':'flat',
     note:[s('today.macro_regime_'+regime),OK(netT)?s('today.macro_net_liquidity',{value:num(netT,2)}):null,OK(change)?s('today.macro_net_change',{value:signed(change)}):null].filter(Boolean).join(' · ')},
    ...
    {key:'fng',label:s('today.macro_fng'),value:OK(fng?.score)?num(fng.score,0):'—',unit:'',tone:OK(fng?.score)?(fng.score<45?'down':fng.score>55?'up':'flat'):'flat',
...
export function macroStrip(doc){
  ...
  for(const tile of macroTiles(doc))grid.append(el('div.today-macro-tile',{'data-tile':tile.key,class:'is-'+tile.tone},
    el('span.today-macro-label',tile.label),el('strong.today-macro-value.mono',tile.value,tile.unit?el('span.today-macro-unit',tile.unit):null),
